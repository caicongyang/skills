...
description: A股放量股分析工具。选出10个交易日内存在成交量翻倍且今日成交量翻倍的股票，按概念板块分组展示。
metadata:
  {
    "openclaw":
      {
...
1. **今日成交量翻倍**：今日成交量 ≥ 2倍昨日成交量
2. **10个交易日内存在成交量翻倍**：过去10个交易日中，任意相邻两天出现过成交量翻倍（day-over-day doubling）

## 输出格式

标题：`📊 放量股分析 {日期} — 10个交易日内存在成交量翻倍且今日成交量翻倍`

按概念板块分组展示，每组最多5只：

### 📊 芯片概念 (X只)
| 代码 | 名称 | 收盘价 | 涨幅 | 今日倍数 | 历史翻倍日 | 历史倍数 |
|:---|:---|:---:|:---:|:---:|:---:|:---:|
| 000001 | 某股票 | 10.50 | +5.20% | 2.35x | 2026-03-25 | 2.10x |
